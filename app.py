...
# File to store system PID
PID_FILE = "system_pid.json"
...
# Function to load PID from file
def load_system_pid():
    try:
...
# Function to check if system is running (enhanced version)
def is_system_running():
    try:
        # First check using saved PID
        pid = load_system_pid()
        if pid and psutil.pid_exists(pid):
            try:
                ...
                if 'main.py' in cmdline:
                    ...
                    # Process exists but it's not our script, clear the PID
                    clear_system_pid()
        ...
        if main_processes:
            # If we found processes but don't have a PID saved, save the first one
            if not pid and main_processes:
                save_system_pid(main_processes[0].pid)
            return True
...
# Function to find main.py processes
def find_main_py_processes():
    """Find all running processes that are executing main.py"""
    processes = []
    try:
...
# Function to stop the system (enhanced version)
def stop_system():
    try:
            ...
            try:
                ...
                if 'main.py' in cmdline:
                    ...
                    try:
                        ...
                        print("System stopped via saved PID")
                    except psutil.TimeoutExpired:
                        process.kill()
                        process.wait()
                        stopped_via_pid = True
                        print("System force killed via saved PID")
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass
        ...
        # If PID method didn't work, try to find and kill by process name
        ...
        # Verify all main.py processes are stopped
        remaining_processes = find_main_py_processes()
        if remaining_processes:
            print(f"Warning: {len(remaining_processes)} main.py processes still running")
            return False
        else:
            print("All main.py processes stopped successfully")
            return True
...
def start_system():
    try:
        # Make sure no existing process is running
        ...
        if is_system_running():
            ...
            return False, "System failed to start properly"
...
def toggle_system():
    try:
        ...
        if current_state:
            ...
            print("Attempting to stop system...")
            if stop_system():
            ...
            print("Attempting to start system...")
        ...
        error_msg = f"Error toggling system: {str(e)}"
        print(error_msg)
        flash(error_msg, "danger")
    
    return redirect(url_for('dashboard'))

# Debug route to check system status (optional - remove in production)
...
def system_debug():
    try:
        ...
        # Get info about all main.py processes
        for proc in find_main_py_processes():
            try:
                debug_info["main_py_processes"].append({
                    "pid": proc.pid,
